...
class ScrollDurationManager(
    ...
    override fun calculateExtraLayoutSpace(state: RecyclerView.State, extraLayoutSpace: IntArray) {
        try {
            val method = mParent?.javaClass?.getDeclaredMethod("calculateExtraLayoutSpace",state.javaClass,extraLayoutSpace.javaClass)
            method?.isAccessible = true
            method?.invoke(method,state,extraLayoutSpace)
